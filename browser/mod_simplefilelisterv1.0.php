...
function SimpleFileBrowserRenderer($params,$forceDirLocation)
{
    $session = JFactory::getSession();
    $baseurl = "";

    $sfl_version = "1.0";
    $sfl_basepath = "plugins/content/simplefilebrowser/browser/tmpl/";

    if($forceDirLocation == '')
        $sfl_dirlocation = $params->get('sfl_dirlocation', '.' . DIRECTORY_SEPARATOR . 'images');
    else
        $sfl_dirlocation = $forceDirLocation;

    $sfl_maxfiles = $params->get('sfl_maxfiles', '20');
    $sfl_bgcolor = $params->get('sfl_bgcolor', '#ffffff');
    ...
    $sfl_boxleft = $params->get('sfl_boxleft', '0');
...
//Security check
        ...
        return SFLAjaxServlet::getContent($_GET["sflaction"], $params, $forceDirLocation,$sfl_dirlocation, $sfl_basepath, $sfl_maxfiles, $sfl_userlocation, $sfl_file);
    } else {

        // include the helper file
        require_once(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'helper.php');
        $results = '';
        $results .= ModSimpleFileListerHelperv10::getFileList($params,$forceDirLocation,$sfl_dirlocation, $sfl_basepath, $sfl_maxfiles, $sfl_userlocation);

        // include the template for display
        //require('mod_simplefilelisterv'.$sfl_version);
        require('tmpl/default.php');
        return SimpleFileBrowserRendererDefault($sfl_dirlocation,$sfl_allowdelete,$sfl_maxheight,$sfl_bgcolor,$sfl_boxleft,$results);
    }
}